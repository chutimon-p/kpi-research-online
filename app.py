...
if menu == "📊 Dashboard & Reports":
    ...
    # --- หัวใจสำคัญ: การนับจำนวนอาจารย์ (ตัวหาร n) ตามไฟล์ Excel ---
    # กรองเอาอาจารย์ที่ "มีหลักสูตร" จริงๆ เท่านั้น (ไม่นับ - หรือค่าว่าง)
    df_master_clean = df_master[
        (df_master['หลักสูตร'].notna()) & 
        (df_master['หลักสูตร'] != "-") & 
        (df_master['หลักสูตร'] != "")
    ].copy()
    prog_member_counts = df_master_clean.groupby("หลักสูตร")["Name-surname"].nunique().to_dict()

    # กรองเอาคณะที่มีจริงเท่านั้น
    df_faculty_clean = df_master[
        (df_master['คณะ'].notna()) & 
        (df_master['คณะ'] != "-") & 
        (df_master['คณะ'] != "")
    ].copy()
    ...
    with t1:
        ...
        # ตรรกะ: 1 ชื่อเรื่องนับ 1 ครั้งต่อหลักสูตร
