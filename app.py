...
if menu == "📊 รายงานและ KPI":
    ...
    if df_master.empty:
        ...
        with t1:
            st.subheader(f"🎓 KPI รายหลักสูตร (จัดกลุ่มตามคณะ) ({year_option})")
            ...
            # --- เรียงตาม คณะ และ คะแนน KPI เพื่อให้คณะเดียวกันอยู่ติดกัน ---
            prog_report = prog_report.sort_values(by=["คณะ", "คะแนน KPI"], ascending=[True, True])

            fig_prog = px.bar(prog_report, x="คะแนน KPI", y="หลักสูตร", color="คณะ", 
                             orientation='h', range_x=[0, 5.5], text="คะแนน KPI", height=750,
        ...
        with t3:
            ...
            if not res_all_time.empty:
                ...
                fig_fac = px.bar(fac_year, x="ปี", y="คะแนน", color="คณะ", barmode="group", text_auto='.2f',
                                labels={"คะแนน": "คะแนนสะสมถ่วงน้ำหนัก", "ปี": "ปี พ.ศ."})
                st.plotly_chart(fig_fac, use_container_width=True)

# ==========================================
# 5. ส่วน Admin (ต้อง Login)
# ==========================================
